# Title     : LT Spacial
# Objective : Spacial memory analysis
...
data <- data[complete.cases(data$choice),]
...
Anova(model <- glmer(choice ~ selection  +trial_centered + assignedR +
  (1|id) ,
                     data = d,
...
d%>%
  ...
  theme_classic()+
  labs(title = "Long term memory", subtitle = "Spacial learning")+
  ylab("proportion of correct choices(SE)")+
  xlab("side trained on")
...
## Explained variance (pseudo-R2 for binomial models) (theoretical R2 for binomial models)
